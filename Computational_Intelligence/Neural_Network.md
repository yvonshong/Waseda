...
Gradient descent is based on the observation that if the multi-variable function $F(x)$ is defined and differentiable in a neighborhood of a point $a$ , then $F(x)$ decreases fastest if one goes from $a$ in the direction of the negative gradient of $F$ at $a$, $-\  nabla F(a)$. It follows that, if $b=a-\gamma \  nabla F(a)$
...
The network has d inputs, m hidden neurons and n output neurons. The transfer function of the
...
https://www.spsc.tugraz.at/sites/default/files/lecturenotes_3.pdfhttps://page.mi.fu-berlin.de/rojas/neural/chapter/K7.pdf
